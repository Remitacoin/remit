...
#include "qt/remita/forms/ui_snackbar.h"
#include "qt/remita/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(remitaGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &remitaGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
