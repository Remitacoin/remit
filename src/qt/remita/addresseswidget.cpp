...
#include "qt/remita/forms/ui_addresseswidget.h"
#include "qt/remita/addresslabelrow.h"
#include "qt/remita/addnewaddressdialog.h"
#include "qt/remita/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(remitaGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination remitaAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(remitaAdd, label.toUtf8().constData(),
