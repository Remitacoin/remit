...
#include "qt/remita/forms/ui_send.h"
#include "qt/remita/addnewcontactdialog.h"
#include "qt/remita/qtutils.h"
#include "qt/remita/sendchangeaddressdialog.h"
#include "qt/remita/optionbutton.h"
#include "qt/remita/sendconfirmdialog.h"
#include "qt/remita/myaddressrow.h"
#include "qt/remita/guitransactionsutils.h"
...
SendWidget::SendWidget(remitaGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-remita");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination remitaAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(remitaAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(remitaAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
