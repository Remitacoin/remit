...
#include "qt/remita/dashboardwidget.h"
#include "qt/remita/forms/ui_dashboardwidget.h"
#include "qt/remita/sendconfirmdialog.h"
#include "qt/remita/txrow.h"
...
DashboardWidget::DashboardWidget(remitaGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareREMIT, "square-chart-remita");
    setCssProperty(ui->labelREMIT, "text-chart-remita");
    ...
    setCssProperty(ui->labelAmountREMIT, "text-stake-remita-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &remitaGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &remitaGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &remitaGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            remita = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalREMIT += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesREMIT.append(remita);

        int max = std::max(remita, remita);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesREMIT);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalREMIT > 0) {
        setCssProperty(ui->labelAmountREMIT, "text-stake-remita");
    } else {
        setCssProperty(ui->labelAmountREMIT, "text-stake-remita-disable");
    }
    forceUpdateStyle({ui->labelAmountREMIT});
    ui->labelAmountREMIT->setText(GUIUtil::formatBalance(chartData->totalREMIT, nDisplayUnit));
