...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::REMIT);
    ...
    if (settings.contains("nAnonymizeremitaAmount") || reset)
        SoftSetArg("-anonymizeremitaamount", settings.value("nAnonymizeremitaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in remita.cpp during shotdown to also remove the window positions
