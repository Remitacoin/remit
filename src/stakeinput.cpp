...
bool CREMITStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CREMITStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CREMITStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CREMITStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CREMITStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CREMITStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CREMITStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CREMITStake::GetUniqueness() const
{
    //The unique identifier for a REMIT stake is the outpoint
...
CBlockIndex* CREMITStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CREMITStake::ContextCheck(int nHeight, uint32_t nTime)
{
