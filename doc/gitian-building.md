...
*Setup instructions for a gitian build of remita using a VM or physical system.*

Gitian is the deterministic build process that is used to build the remita
...
to the remita GitHub release page.
...
- [Building remita](#building-remita-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/remita-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/remita-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/remita-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/remita-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/remita-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/remita-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/remitavps-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@remita.trade) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/remita-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/remita-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/remita-osx-*-build.assert
```
